...
interface ClienteFormProps {
  ...
  onSubmit: (valores: ClienteInput) => void
  enviando: boolean
}

export function ClienteForm({ clienteInicial, onSubmit, enviando }: ClienteFormProps) {
  const {
    register,
    handleSubmit,
    formState: { errors },
    ...
    >
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <Campo label="Tipo de documento" error={errors.tipoDocumento?.message}>
          <select {...register('tipoDocumento')} className={inputClass}>
